...
# This code creates a 'completeness' variable which tells how much data we have available to make up the entire MSA for the outcome:
...
# 2. what counties we have female population data for (x) (assign them an MSA)
...
#2 
female.population.counties = as.data.frame.table(data.manager$data$female.population.denominator.for.fertility.rate$estimate$cdc.wonder.natality$cdc.fertility$year__location__age__race__ethnicity)

female.population.counties <- female.population.counties %>%
    filter(location != "US")%>% #Remove US
    mutate(outcome = "counties we have female population data for")%>%
    select(-Freq)%>%
    mutate(county.we.have.female.pop.for = "1")%>%
    select(-age, -race, -ethnicity) #Removing stratifications so their corresponding county populations aren't summed multiple times in the steps below

female.population.counties<- female.population.counties[!duplicated(female.population.counties), ] #then remove duplicates 
...
everything_v1 = left_join(female.population.counties, county.population, by=c("year", "location"))
...
#Format for put:
final.completeness.female.population <- everything_v4%>%
    rename(location = msa)%>%
    rename(value = completeness)%>%
    select(year, location, value)%>%
    mutate(year = as.character(year))%>%
    mutate(value = as.numeric(value))%>%
    mutate(outcome = "completeness.female.population.denominator.for.fertility.rate")

final.completeness.female.population <- as.data.frame(final.completeness.female.population)
...
data.manager$put.long.form(
    data = final.completeness.female.population,
