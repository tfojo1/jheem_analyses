...
#Plot1- Washington, DC
plot.one <- all.dx.prev.source1 %>% filter(location == "C.47900")
ggplot(data = plot.one)+                   
  ...
  theme()+
  ylim(0, NA)+
  ggtitle("Diagnosed Prevalence, Washington, DC (C.47900), cdc.surveillance.reports")

#Plot2- Riverside-San Bernardino, CA
plot.two <- all.dx.prev.source1 %>% filter(location == "C.40140")
ggplot(data = plot.two)+                   
  ...
  theme()+
  ylim(0, NA)+
  ggtitle("Diagnosed Prevalence, Riverside-San Bernardino, CA (C.40140), cdc.surveillance.reports")
...
dx.prev.ehe <- run.outlier.process(outcome= 'diagnosed.prevalence',
                                   stratifications= list(c()), 
                                   data.manager= surveillance.manager,
                                   phi=0.3, #default is phi=0.15 (percent change from year to year)
                                   theta=0.1, #default is theta=0.05 (a multiplier that produces a percent change)
                                   locations= c(EHE.MSAS))
