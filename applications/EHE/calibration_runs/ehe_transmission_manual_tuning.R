...
# load initial transmission mcmc for NYC
load("../jheem_analyses/prelim_results/init.transmission.sim_2024-02-01_C.35620.Rdata") 

# rerun sim with parameters from mcmc, because we have changed parameters since the mcmc was run (specifically, aging rates)
...
# check to make sure they have diff outputs, even though they use the same params 
simplot(sim, sim.mcmc.0,
        facet.by = "risk", 
        outcomes = c("new"),
...
simplot(sim.mcmc.0, sim.manual,
        facet.by = "risk", 
        outcomes = c("new"),
        dimension.values = list(year = 1981:2020)) # look from 1981 to start, then zoom in from 2008

simplot(sim.mcmc.0, sim.manual,
        facet.by = "risk", 
        outcomes = c("aids.diagnoses"),
        dimension.values = list(year = 1981:2000))

# can't increase black and hispanic msm without completely inflating other msm 
simplot(sim.mcmc.0, sim.manual,
