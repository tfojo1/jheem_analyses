...
files.old = files[grepl("init.pop.migration.sim_2024-01-23",files)]
# files.old = c(files.old.1,files.old.2,files.old.3,files.old.4,files.old.5)
locations.old = str_sub(files.old,-13,-7)
names(locations.old) = names(LOCATIONS.OF.INTEREST[match(locations.old,unname(LOCATIONS.OF.INTEREST))])


# files = files[grepl("init.pop.migration.sim_2024-01-23",files)]
files = files[grepl("init.pop.migration.sim_2024-01-25",files)]
...
for(i in 1:length(locations.run)){
  ...
  if(length(old.file)>0){
    ...
    plot = simplot(sim.new, "population",
                   facet.by = "age",split.by = "race",
                   dimension.values = list(year = as.character(2000:2020))) + 
  ...
  ggsave(filename = 
           paste0("prelim_results/2024_01_25_figs/",names(location),".jpeg"),
...
simplot(sim, outcomes = c("new","diagnosed.prevalence"),
        dimension.values = list(year = as.character(2000:2020)))
...
# dev.off()
# sim.manual$get(outcomes = "new",keep.dimensions = "year")