...
get.sexual.susceptibility.by.age <- function(location, specification.metadata, 
                                             population.years = DEFAULT.POPULATION.YEARS,
                                             ref.age.index = ceiling(specification.metadata$n.ages/2))
{
  ...
  # from table 2
    ...
    if (is.null(pop))
      stop("There was no population data at all for location ", location, " between ", min(population.years), " and ", max(population.years))
    
    pop = apply(pop, 'age', sum)
    pop.parsed.ages = parse.age.strata.names(names(pop))
    max.age = max(pop.parsed.ages$lower)+1
    ...
    aggregate.frequency.times.availability = sapply(1:specification.metadata$n.ages, function(a){
      
        indices = specification.metadata$age.lower.bounds[a]:min(specification.metadata$age.upper.bounds[a], max.age-1) 
        #sum(single.year.frequency.times.availability[indices]) / length(indices)
        sub.freq = single.year.frequency.times.availability[indices - min.age + 1]
        sub.pop = pop[indices + 1]
        
        sum(sub.freq * sub.pop) / sum(sub.pop)
    })
    
    array(aggregate.frequency.times.availability / aggregate.frequency.times.availability[ref.age.index],
          dim = c(age=specification.metadata$n.ages),
          dimnames = list(age.to=specification.metadata$dim.names$age))
}

get.idu.susceptibility.by.age <- function(location, specification.metadata, 
                                             population.years = DEFAULT.POPULATION.YEARS,
                                             ref.age.index = ceiling(specification.metadata$n.ages/2))
{
