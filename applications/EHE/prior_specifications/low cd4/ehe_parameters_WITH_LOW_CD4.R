...
BASE.PARAMETERS.PRIOR = join.distributions(
    ...
    low.cd4.symptomatic.testing.rate = Lognormal.Distribution(-0.5692455, 0.1034369), # see ehe_parameter_calculations.R for citations and derivation
    acute.symptomatic.testing.rate = Lognormal.Distribution(0.1468349), # see ehe_parameter_calculations.R for citations and derivation
    ...
    #-- Low-CD4 (ie AIDS) parameters --#
    low.cd4.no.art.mortality.rate = Lognormal.Distribution(log(0.3329317), 0.1796032), # see ehe_parameter_calculations.R for citations and derivation
    low.cd4.with.art.mortality.rate = Lognormal.Distribution(log(0.04584686), 0.1635133), # see ehe_parameter_calculations.R for citations and derivation
    cd4.decline.rate = Lognormal.Distribution(-2.448311, 0.1348876), # see ehe_parameter_calculations.R for citations and derivation
    cd4.recovery.rate = Lognormal.Distribution(log(0.8632973), log(1.239383) / 2), # see ehe_parameter_calculations.R for citations and derivation
...
EHE.PARAMETERS.PRIOR = join.distributions(
 ...
 #   low.cd4.suppressed.slope.or = Lognormal.Distribution(0, 0.5 * log(2) /
 #                                                          5)
)
...
# = Normal.Distribution(0,1),
  ...
  symptomatic.testing.and.ramp.up = c('low.cd4.symptomatic.testing.rate',
                                      'acute.symptomatic.testing.rate',
  ...
  low.cd4 = c('low.cd4.no.art.mortality.rate',
              'low.cd4.with.art.mortality.rate',
              'cd4.decline.rate',
              'cd4.recovery.rate'),
...
EHE.PARAMETER.SAMPLING.BLOCKS = 
  c(BASE.PARAMETER.SAMPLING.BLOCKS,
    list(
      ...
      #low.cd4.suppression = 'low.cd4.suppressed.slope.or'
    ))
