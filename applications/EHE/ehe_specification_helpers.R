                                                   ...
                                                   keep.ages = F)
{
    ...
    #proportion.msm.by.location = ALL.DATA.MANAGERS$msm.proportions[as.character(as.numeric(fips))]
...
get.testing.functional.form = function(location, specification.metadata, 
  ...
  # once we get the functional form; have it predict out values to see trends 
  #values = testing.functional.form$project(2015:2035) # list with one array for each year
  # plot average value for each age; make sure it's not doing anything crazy; plot with one line for each age 
  # also take max and min; check for crazy values
