...
#       nohup bash applications/SHIELD/launchers/launch_interventions.sh > /dev/null 2>&1 &
#   Kill Runs:
...
#       tail -f /home/jheem-shared/logs/launcher_interventions_<user>.out
#
#   Check a specific city+calibration log:
#       tail -f /home/jheem-shared/logs/<loc>_<calib_code>_interventions.out
#
# HOW IT WORKS
...
#   Check /home/jheem-shared/logs/<loc>_<calibration_code>_interventions.out for the R-level error message.
...
#   2. Watch with:   tail -f $LOG_DIR/launcher_interventions_<user>.out
...
[[ -t 1 ]] || exec > "$LOG_DIR/launcher_interventions_${USER:-$(id -un)}.out" 2>&1
...
run_calib_code() {
    ...
    Rscript "$SCRIPT" "$loc" "$calib_code" "$N_SIM" "$FIRST_YEAR" "$LAST_YEAR" \
        > "$LOG_DIR/${loc}_${calib_code}_interventions.out" 2>&1
    local rc=$?
    if (( rc != 0 )); then
...
for loc in "${CITIES[@]}"; do
    for calib_code in "${CALIBRATION_CODES[@]}"; do
        ...
        echo "[$(date '+%F %T')] LAUNCHED INTERV $loc :: $calib_code (PID $!, running_jobs=$(running_slots))"
