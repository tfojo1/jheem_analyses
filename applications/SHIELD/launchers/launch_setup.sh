...
#
# STEP 1 of 4: SETUP
#   Sets up the calibration cache/chains for each city, for each calibration code.
#   Run this FIRST. After it completes, manually inspect the logs and the
#   calibration cache directories before moving on to launch_run_chains.sh.
#
# USAGE
#   nohup bash applications/SHIELD/launchers/launch_setup.sh > /dev/null 2>&1 &
#
# Kill:
...
#
# Monitor:
#   tail -f /home/jheem-shared/logs/launcher_setup_<user>.out
...
#   2. Watch with:   tail -f $LOG_DIR/launcher_setup_<user>.out
...
[[ -t 1 ]] || exec > "$LOG_DIR/launcher_setup_${USER:-$(id -un)}.out" 2>&1
...
MAX_JOBS=30 #Depends on the machine
...
# ── per city+calibration code setup job ────────────────────────────────────────
run_setup() {
    local loc="$1"
    local calib_code="$2"
    echo "[$(date '+%F %T')] START   SETUP $loc :: $calib_code"
    Rscript "$SCRIPT" "$loc" "$calib_code" setup \
        > "$LOG_DIR/${loc}_${calib_code}_setup.out" 2>&1
    local rc=$?
    if (( rc != 0 )); then
        echo "[$(date '+%F %T')] FAILED  SETUP $loc :: $calib_code (exit $rc)" >&2
        return 1
    fi
    echo "[$(date '+%F %T')] DONE    SETUP $loc :: $calib_code"
}
...
wait
echo "[$(date '+%F %T')] ALL SETUP JOBS DONE"
echo "[$(date '+%F %T')] >>> Manually verify the calibration cache/setup logs before running launch_run_chains.sh <<<"
