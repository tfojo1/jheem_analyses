...
#       nohup bash applications/SHIELD/launch_shield_multiprocess.sh > /dev/null 2>&1 &
#   Kill Runs: 
...
#       tail -f /home/jheem-shared/logs/launcher_multiprocess_<user>.out
#
#   Check a specific city+stage log:
#       tail -f /home/jheem-shared/logs/C.19100_calib.4.24.stage1.az.out
#
# HOW IT WORKS
...
#   Check /home/jheem-shared/logs/<loc>_<stage>.out for the R-level error message.
...
# LOG_DIR="$SCRIPT_DIR/logs"          # previous setup: logs/ inside the repo
...
#   2. Watch with:   tail -f $LOG_DIR/launcher_multiprocess_<user>.out
...
[[ -t 1 ]] || exec > "$LOG_DIR/launcher_multiprocess_${USER:-$(id -un)}.out" 2>&1
