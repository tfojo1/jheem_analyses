...
#   Table 1    Baseline epidemiology, no intervention    1-notInt.csv
#   Table 2    Impact at one coverage, Total + MSM       2-impact-cov<c>-<yr>.csv
#   Table 3    Efficiency: person-years of doxycycline   3-efficiency.csv
#              per infection averted; Total/MSM/female
#   Table 4    Spillover and its candidate drivers       4-spillover-cov<c>.csv
#   Table 5    Driver associations (descriptive)         5-spillover-associations-cov<c>.csv
#   Figure 1   Baseline trends, 4 panels                 fig1_noint.png
#   Figure 2   Impact heatmaps, 2x2                      fig2_heatmaps.png
#   Figure 3   Critical coverage, 2x2                    fig3_crit_coverage.png
#   Figure 4   Spillover vs four candidate drivers       fig4_spillover_drivers.png
#
# FIGURE 1 PANEL ORDER is deliberate and not alphabetical:
#   (p1A | p1B) / (p1D | p1C), so the panel TAGGED C is the P&S diagnosis rate
#   and the panel TAGGED D is the MSM share. Cite them by tag, not by object
#   name.
#
...
# ****************************************************************************************************
# FIGURE 1 -- NO INTERVENTION TRENDS -----
# ****************************************************************************************************
...
p1A <- plot_trend_by_location(fig1.tbl, outcome = "incidence",
                             subgroup = "Total", 
                             # dashed = HL,
                             log.y = TRUE,
...
p1B <- plot_trend_by_location(fig1.tbl, outcome = "rate_incidence_per_pop",
                             subgroup = "Total", 
                             # dashed = HL,
                             y.lab = "Incidence rate (per 100,000 person-years)")


p1C <- plot_trend_by_location(fig1.tbl, outcome = "pct_incidence_share",
                             subgroup = "msm", 
                             # dashed = HL,
                             y.lab = "MSM share of incident infections (%)")

p1D <- plot_trend_by_location(fig1.tbl, outcome = "rate_diagnosis_ps_per_pop",
                             subgroup = "Total", 
                             # dashed = HL,
                             y.lab = "PS Diagnosis rate (per 100,000 person-years)")

fig1 <- (p1A | p1B)/( p1D | p1C) + plot_annotation(tag_levels = "A");fig1
...
p2A <- mk(tbl.rel.2022, "msm",   "A: MSM (vs. 2022 baseline)")
p2B <- mk(tbl.rel.2022, "Total", "B: Total Population (vs. 2022 baseline)")
p2C <- mk(tbl.rel.2030,          "msm",   "C: MSM (vs. 2030 No Intervention)")
p2D <- mk(tbl.rel.2030,          "Total", "D: Total Population (vs. 2030 No Intervention)")
...
# ****************************************************************************************************
# FIGURE 3 -- DOXY CRITICAL COVERAGE ----
# ****************************************************************************************************
...
# TABLE 2 -- DOXY IMPACT: SINGLE COVERAGE: TOTAL ---- 
...
# ****************************************************************************************************
{
    ...
    col.labels.t2 <- stats::setNames(
        c("MSA", "Population",
          ...
          paste0("Incidence Rate (per 100,0000 population) ", BASE.YEAR, ", No Intervention"),
          paste0("Incidence Rate (per 100,0000 population) ", EVAL.YEAR, ", No Intervention"),
          paste0("Incidence Rate (per 100,0000 population) ", BASE.YEAR, ", Doxy Coverage ",.t2cov,"%"),
          paste0("Incidence Rate (per 100,0000 population) ", EVAL.YEAR, ", Doxy Coverage ",.t2cov,"%"),
    ...
    write.csv(final2,
              file = paste0(TABLE.DIR, "2-impact-cov", .t2cov, "-", EVAL.YEAR, ".csv"),
              row.names = FALSE)
}

# ****************************************************************************************************
# TABLE 3 -- DOXY-PEP EFFICIENCY ----
# ****************************************************************************************************
#
...
# ****************************************************************************************************
# TABLE 4 -- SPILLOVER AND ITS DRIVERS ----
# ****************************************************************************************************
...
write.csv(final4, file.path(TABLE.DIR, paste0("4-spillover-cov", .ccrit, ".csv")),
          row.names = FALSE)
...
# ****************************************************************************************************
# TABLE 5: CANDIDATE DRIVERS OF THE SPILLOVER (DESCRIPTIVE) -----
# ****************************************************************************************************
#
...
# ****************************************************************************************************
# FIGURE 4 -- SPILLOVER AGAINST FOUR CANDIDATE DRIVERS ----
# ****************************************************************************************************
{
...
ggsave(file.path(FIG.DIR, "fig4_spillover_drivers.png"), fig4,
