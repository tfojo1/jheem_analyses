...
if (1==2){
    ...
    ##############################
    # Boxplots of total encounter by age group
    ##############################
    #total number of encounters
    # <<< Those in the 25-29 age category have largest frequency of encounters >>>
    ...
    # people who have any partners
    # <<< Those in the 25-29 age category have largest frequency of partnerships >>>
    ggplot(GSS_n_sex_part_by_age_sex, aes(x = age_group, y = `Any Partner`, fill = Sex)) +
        geom_boxplot(outlier.shape = 16, outlier.size = 1.5, alpha = 0.7) +
        labs(
            title = "General Social Survey (Annual GSS 1972-2024)",
            x = "Age Group",
    ...
    ##############################
    # Boxplots of percent values by age group
    ##############################
    ...
    ##############################
    # Boxplots with weighted means overlaid
    ##############################
    ...
    ##############################
    # Compute average number of partners by age and sex
    ##############################
    
    GSS_activity <- GSS_n_sex_part_by_age_sex %>%
        mutate(
            weighted_sum =
                `No partners` * 0 +
                `1 partner` * 1 +
    ...
    ##############################
    # Construct sexual activity score (partners × any-sex probability)
    ##############################
    GSS_activity <- GSS_activity %>%
        left_join(
            GSS_anysex_summary,
    ...
    ##############################
    # Normalize sexual activity within sex (ref = age 20–24)
    ##############################
    ...
    GSS_activity <- GSS_activity %>%
        ...
        mutate(
            ...
            # Normalize
            sexual_activity_normalized_20_24 = sexual_activity / ref_act
        )
    
    ##############################
    # Plot sexual activity index across age groups
    ##############################
    
    ggplot(GSS_activity, aes(x = age_group, y = sexual_activity_normalized_20_24, color = Sex, group = Sex)) +
    ...
    het_sexualActivity_means=GSS_activity[GSS_activity$Sex=="Female",]$sexual_activity_normalized_20_24
    msm_sexualActivity_means=GSS_activity[GSS_activity$Sex=="Male",]$sexual_activity_normalized_20_24
}
