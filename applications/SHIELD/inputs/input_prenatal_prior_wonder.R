...
lapply(c(1,2,3),function(x){
  if (x==1) df$P <- df$p1;trimester = '1st Trimester'
  if (x==2) df$P <- df$p2.exc;trimester = '2nd Trimester'
  if (x==3) df$P <- df$p3.exc;trimester = '3rd Trimester'
  df$P[df$P>0.99]<-0.99 #to avoid infinite values in the logit
  selected.model= "fully.interacted"
  anchor.year = 2016
  df$year = df$year-anchor.year
  #
  prenatal.prior = get.intercepts.and.slopes(df, model=selected.model)
  # Cache this prior for future use in SHIELD
  cache.object.for.version(object = prenatal.prior, 
                           name = paste0("prenatal.prior.trimester",x),
                           version = 'shield', 
                           overwrite=T)
  print(paste0("Cashed object ","prenatal.prior.trimester",x))
})
