# Make supplemental table for median age by race per state
...
race_totals <- get(load(file="../jheem_analyses/applications/age_analysis/Rdata Objects/med_age_timeline_race_data.Rdata"))
race_delta_totals <- get(load(file="../jheem_analyses/applications/age_analysis/Rdata Objects/med_age_delta_race_data.Rdata"))
load(file="../jheem_analyses/applications/age_analysis/Rdata Objects/med_age_timeline_race_loc_data.Rdata")
load(file="../jheem_analyses/applications/age_analysis/Rdata Objects/med_age_delta_race_loc_data.Rdata")
race_data <- rbind(med_age_timeline_race_data,
                   race_totals %>%
                       mutate(location="total") %>%
                       relocate(location, .after = year))
race_delta_data <- rbind(med_age_delta_race_data,
                         race_delta_totals %>%
                             mutate(location="total") %>%
                             relocate(location, .before = race))
...
fake_data <- filter(race_data, year%in%c('2025', '2040')) %>%
    select(year, location, race, mean) %>%
    pivot_wider(names_from = year, values_from = mean) %>%
    mutate(fake_delta = `2040` - `2025`)
race_delta_data <- race_delta_data %>%
...
table_states <- cbind(do_conversion_operations(filter(race_data, race=="black")),
                      do_delta_conversion_operations(filter(race_delta_data, race=="black")),
                      do_conversion_operations(filter(race_data, race=="hispanic")),
                      do_delta_conversion_operations(filter(race_delta_data, race=="hispanic")),
                      do_conversion_operations(filter(race_data, race=="other")),
                      do_delta_conversion_operations(filter(race_delta_data, race=="other")))
...
do_create_table_colors <- function(df) {
    my_cols <- matrix(as.numeric(df[2 * seq_along(state_order) - 1, 3 * (1:3)]), ncol = 3)
    pos <- my_cols > 0
    neg <- my_cols < 0
...
# do_create_table_colors <- function(df, num.groups=3, groups.with.percent=numeric(0), groups.with.comma=numeric(0), with.total.prev=F) {
#     browser()
...
write.shaded.table(csv_double_rows,
                   file = "../jheem_analyses/applications/age_analysis/Figures/Manuscript/race_shaded_table.xlsx",
